  ...
  return (
    <section className="py-32 px-6 bg-[#F7F4F0]">
      <div className="max-w-6xl mx-auto">
        ...
        <ScrollReveal>
          <div className="mt-12 mb-16 rounded-2xl overflow-hidden shadow-[0_8px_40px_rgba(27,58,75,0.1)]">
            <img
              src="/images/surfers.jpg"
              alt="Three surfers walking into the ocean at golden hour"
              className="w-full h-[280px] md:h-[360px] object-cover object-center"
            />
          </div>
        </ScrollReveal>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-10 mt-32">
          {founders.map((founder, index) => (
            <ScrollReveal key={founder.name} delay={index * 0.1}>
              <div className="relative pt-16 p-10 border-t-2 border-[#C45D3E] bg-[#FDFCFA] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_16px_48px_rgba(27,58,75,0.08)]">
                <div className="absolute -top-14 left-1/2 -translate-x-1/2">
                  <img
                    src={founder.photo}
                    alt={founder.name}
                    className="w-28 h-28 rounded-full object-cover object-center border-4 border-[#FDFCFA] shadow-[0_4px_20px_rgba(27,58,75,0.12)]"
                  />
                </div>
                <h3 className="font-serif text-4xl text-[#1B3A4B] font-normal mb-1 text-center">
                  {founder.name}
                </h3>
                <p className="text-sm tracking-wide text-[#C45D3E] font-medium mb-5 text-center">
                  {founder.role}
                </p>
